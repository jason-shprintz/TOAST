...
export class CoreStore {
  ...
  async createNote(params: {
    ...
    /**
     * Creates a new `Note` object with a unique ID, timestamp, and provided parameters.
     *
     * @remarks
     * - If both `latitude` and `longitude` are defined, they are included in the note.
     * - The `category` defaults to `'General'` if not specified in `params`.
     * - Initializes `photoUris` as an empty array.
     *
     * @property {string} id - Unique identifier for the note, generated by `generateId()`.
     * @property {number} createdAt - Timestamp of note creation (milliseconds since epoch).
     * @property {number} [latitude] - Optional latitude coordinate, included if defined.
     * @property {number} [longitude] - Optional longitude coordinate, included if defined.
     * @property {string} category - Category of the note.
     * @property {string} type - Type of the note, as specified in `params`.
     * @property {string} text - Text content of the note.
     * @property {string} [sketchDataUri] - Optional sketch data URI.
     * @property {string[]} photoUris - Array of photo URIs, initialized as empty.
     */
    const note: Note = {
      id: this.generateId(),
      createdAt: Date.now(),
